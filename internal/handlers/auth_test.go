...
func TestHandler_Login(t *testing.T) {
	type mockBehavior func(s *mock_service.MockAuthorization, user models.UserLogin)

	testTable := []struct {
		name                string
		inputBody           string
		inputUserLogin      models.UserLogin
		mockBehavior        mockBehavior
		expectedStatusCode  int
		expectedRequestBody string
	}{
		{
			...
			mockBehavior: func(s *mock_service.MockAuthorization, user models.UserLogin) {
				s.EXPECT().GenerateJWTToken(user, 0).Return("test", nil)
			},
			expectedStatusCode:  200,
			expectedRequestBody: `{"token":"test"}`,
		},
	}
	...
	for _, testCase := range testTable {
		t.Run(testCase.name, func(t *testing.T) {
			...
			auth := mock_service.NewMockAuthorization(c)
			testCase.mockBehavior(auth, testCase.inputUserLogin)
